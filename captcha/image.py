...
class _Captcha(object):
    def generate(self, chars, _format='png'):
        im = self.generate_image(chars)
        out = BytesIO()
        im.save(out, format=_format)
        out.seek(0)
        return out

    def write(self, chars, output, _format='png'):
        im = self.generate_image(chars)
        return im.save(output, format=_format)
...
class ImageCaptcha(_Captcha):
    ...
    def create_captcha_image(self, chars, color, background):
        ...
        def _draw_character(cc):
            ...
            _w, _h = draw.textsize(cc, font=font)

            dx = random.randint(0, 4)
            dy = random.randint(0, 6)
            im = Image.new('RGBA', (_w + dx, _h + dy))
            ...
            # warp
            dx = _w * random.uniform(0.1, 0.3)
            dy = _h * random.uniform(0.2, 0.3)
            ...
            im = im.resize((w2, h2))
            return im.transform((_w, _h), Image.QUAD, data)
        ...
        for _im in images:
            ...
            _new_table = list()
            for _ in table:
                _new_table.append(int(_))

            mask = _im.convert('L').point(_new_table)
            image.paste(_im, (offset, int((self._height - h) / 2)), mask)
